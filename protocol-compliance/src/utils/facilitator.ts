...
export class RSFacilitatorHandle {
  ...
  static async spawn(port?: number, configFilepath?: string) {
    ...
    const resolvedConfig = configFilepath ?? await (async () => {
      const { makeFacilitatorConfig } = await import("./config.js");
      return makeFacilitatorConfig();
    })();
