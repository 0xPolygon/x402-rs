...
where
    ...
    A::Error: IntoResponse,
{
    let json = body.clone().into_json();
    let network = json.get("paymentPayload").and_then(|p| p.get("network")).and_then(|n| n.as_str()).unwrap_or("unknown").to_string();
    let scheme = json.get("paymentPayload").and_then(|p| p.get("scheme")).and_then(|s| s.as_str()).unwrap_or("unknown").to_string();
