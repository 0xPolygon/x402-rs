...
impl<'de> Deserialize<'de> for ChainId {
        ...
        D: Deserializer<'de>,
    {
        ...
        // Resolve env var references in the chain ID string.
        // Supports: "eip155:$CHAIN_ID", "$FULL_CHAIN_ID", "eip155:${CHAIN_ID}"
        let resolved = resolve_env_vars_in_chain_id(&s).map_err(de::Error::custom)?;

        ChainId::from_str(&resolved).map_err(de::Error::custom)
    }
}

/// Resolves `$VAR` or `${VAR}` references within a chain ID string.
///
/// Examples:
/// - `"eip155:$CHAIN_ID"` → resolves `$CHAIN_ID` from env
/// - `"$FULL_CHAIN_ID"` → resolves the entire string from env
/// - `"eip155:80002"` → returned as-is (no env vars)
fn resolve_env_vars_in_chain_id(s: &str) -> Result<String, String> {
    if !s.contains('$') {
    ...
    while let Some(start) = result.find("${") {
        let end = result[start..]
            .find('}')
            .ok_or_else(|| format!("Unclosed ${{}} in chain ID: {}", s))?
            + start;
        let var_name = &result[start + 2..end];
        let value = std::env::var(var_name).map_err(|_| {
            format!(
                "Environment variable '{}' not found (referenced in chain ID '{}')",
                var_name, s
            )
        })?;
    ...
    // Replace $VAR patterns (alphanumeric + underscore)
    while let Some(start) = result.find('$') {
        let var_chars: String = result[start + 1..]
            .chars()
            .take_while(|c| c.is_alphanumeric() || *c == '_')
            .collect();
        if var_chars.is_empty() {
            return Err(format!("Invalid env var reference in chain ID: {}", s));
        }
        let value = std::env::var(&var_chars).map_err(|_| {
            format!(
                "Environment variable '{}' not found (referenced in chain ID '{}')",
                var_chars, s
            )
